...
public class ContextSwitch {
    ...
    private static void unblock (List < SimProcess > blockedProc, List < SimProcess > readyProcList){
        ArrayList<Integer> unblock = new ArrayList<>();
        for (int index = 0; index < blockedProc.size(); index++) {
        ...
        for (int index = 0; index < blockedProc.size(); index++) {
            SimProcess currentBlockedProc = blockedProc.get(index);
            readyProcList.add(currentBlockedProc);
            blockedProc.remove(index);
        }
    }
